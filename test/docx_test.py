...
"""Single-file DOCX extract -> edit -> generate API runner.

Targets sample3.docx by default and exercises all edit operations when possible:
  - Generic pointer ops: add, replace, remove
  - Paragraph ops: replace_text, insert_paragraph_after,
                   remove_paragraph, remove_empty_paragraphs
  - Table ops: insert_table_after, remove_table,
               insert_table_row, remove_table_row,
               insert_table_column, remove_table_column

Run:
    cd temp-doc/test
    python3 docx_test.py
    python3 docx_test.py --file sample3.docx --output-format docx
"""
...
def _build_generic_ops(
    ...
    # replace title-like first paragraph if present
    if paragraphs:
        instructions.append(
            {
                "op": "replace",
                "path": "/paragraphs/0/text",
                "value": "DOCX Pipeline Test Document - Edited by docx_test.py",
            }
        )
    else:
        print("  [WARN] replace /paragraphs/0/text skipped: no paragraphs extracted")

    # remove first run from paragraph 2 when possible
...
def _build_table_ops(
    ...
    # replace top-left cell in up to 3 tables
    for idx in range(min(3, len(tables))):
        instructions.append(
            {
                "op": "replace",
                "path": f"/tables/{idx}/rows/0/cells/0/text",
                "value": f"Table{idx}_Header_Updated",
            }
        )

    # row and column operations on table 0
    instructions.append(
        {
            "op": "insert_table_row",
            "path": "/tables/0",
            "index": 0,
            "value": "Inserted row by docx_test.py",
        }
    )
    instructions.append(
        {
    ...
    instructions.append(
        {
            ...
            "value": [
                ...
                ["extract-edit-generate", "docx round-trip", "inserted"],
            ],
        }
    )
...
def _build_docx_instructions(extracted: dict[str, Any]) -> list[dict[str, Any]]:
    instructions: list[dict[str, Any]] = []
...
def run_docx_pipeline(
    ...
    with httpx.Client(timeout=180) as client:
        ...
        instructions = _build_docx_instructions(extracted_data)
        if not instructions:
            raise ValueError(
        ...
        generated_title = f"Edited output for {file_name}"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run single-file DOCX extract->edit->generate pipeline "
    ...
    parser.add_argument(
        ...
        help="Format for generated output (default: docx)",
    )
    parser.add_argument(
        "--include-media",
        action="store_true",
